...
public class VerifyCodeController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = prop.getDispatcherValue(VERIFICATION_PAGE);
        try {
            String code = request.getParameter("txtCode");
            HttpSession session = request.getSession();
            String serverCode = (String) session.getAttribute("VERIFICATION_CODE");
            if (code.equals(serverCode)) {
                UserDTO userDTO = (UserDTO) session.getAttribute("DTO");
                if (userDTO != null) {
                    UserDAO dao = new UserDAO();
                    if(dao.updateUserStatus(userDTO.getEmail(), UserDTO.ACTIVE_STATUS)) {
                        userDTO.setStatus(UserDTO.ACTIVE_STATUS);
                ...
                request.setAttribute("INVALID", "Invalid verification code. Try again");
            }
        } catch (Exception e) {
